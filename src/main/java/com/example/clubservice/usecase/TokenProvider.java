package com.example.clubservice.usecase;
...
public class TokenProvider {
    ...
    public TokenResponse generateTokenResponse(Authentication authentication) {
        ...
        String memberUid = authDetails.getMember().getPhone();

        long now = (new Date()).getTime();

        Date accessTokenExpiresIn = new Date(now + ACCESS_TOKEN_EXPIRE_TIME);
        String accessToken = Jwts.builder()
                .setSubject(memberUid)
    ...
    public Authentication getAuthentication(String accessToken) {
        ...
        String memberUid = claims.getSubject();
        Member member = memberRepo.findById(memberUid)
                .orElseThrow(() -> new CustomException(ErrorCode.USER_NOT_FOUND));

        AuthDetails principal = new AuthDetails(member);
